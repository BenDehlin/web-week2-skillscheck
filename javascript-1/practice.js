...
let me = {
	...
	age: 27,
	greeter: function(){
		return `Hello! my name is ${me.firstName} and I love in ${me.state}`
	}
}
...
const filtered = myNumbers.filter(x => x>100)
...
const total = myNumbers.reduce((tot, curr) => tot + curr)
...
const enemies = people.filter(x => x.friend === false)
...
let totallyAwesome = people.map(x => x.awesomeLevel).reduce((acc, curr) => acc + curr)
